...
def SURRec():    
    ...
    if obj_ref:
        ...
        if rs.ObjectType(obj_ref) == 4096:
            ...
            rs.MessageBox('提示：暂不支持直接对图块进行操作，需进入图块编辑模式')
            return
        else:
            ...
            if ci.Index < 0:
                ...
                if line :
                    if rs.IsCurveClosed(line):
                        rs.Command('Noecho _splitface selid %s _Enter C selid %s _Enter'%(srf_id, line[0]),False)
                ...
                if obj_id:
                    ...
                    if line:
                        rs.Command('_Noecho _CPlane _Undo',False )
                        
                        if rs.IsCurveClosed(line):
                            if rs.Command('Noecho _split selid %s _Enter selid %s _Enter'%(obj_id, line[0]),False):
                                spface = rs.LastCreatedObjects(select = True)
                                if spface:
                                    rs.Command('Noecho join',False)                                    
                                    rs.ObjectLayer(line,'CurveOnFace')
                        else:
                            pass
